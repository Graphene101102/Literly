...
const VietnameseKnowledge = () => {
  ...
  const fetchDocuments = async () => {
    try {
      const { data } = await api.get('/documents?category=KNOWLEDGE');
      setDocuments(data);
    } catch (error) {
      console.error(error);
    } finally {
  ...
  return (
    <div className="h-screen w-screen flex flex-col bg-blue-50">
      ...
      <div className="flex-grow relative overflow-y-auto p-8 z-10 flex justify-center">
        <div className="w-full max-w-5xl bg-white bg-opacity-95 rounded-2xl shadow-xl border border-green-100 overflow-hidden h-fit mb-20">

          {/* Header */}
          <div className="bg-gradient-to-r from-green-500 to-green-400 px-8 py-6 flex items-center shadow-md">
            ...
            <div>
              <h2 className="text-3xl font-bold text-white">Kiến thức tiếng Việt</h2>
              <p className="text-green-100 mt-1">Các bài học và kiến thức ngôn ngữ</p>
            </div>
          </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {documents.map((doc) => (
                  <div
                    ...
                    className="group cursor-pointer bg-white hover:bg-green-50 border border-gray-200 hover:border-green-300 p-6 rounded-2xl transition-all duration-300 hover:shadow-lg hover:-translate-y-1 flex flex-col items-center text-center relative overflow-hidden"
                  >
                    <div className="absolute top-0 right-0 w-16 h-16 bg-green-500 rounded-bl-full -mr-8 -mt-8 opacity-10 group-hover:opacity-20 transition-opacity"></div>

                    <div className="bg-green-100 p-4 rounded-full mb-4 shadow-sm group-hover:bg-green-200 group-hover:scale-110 transition-all duration-300">
                      <FileText className="text-green-600" size={32} />
                    </div>

                    <h3 className="font-bold text-gray-800 text-lg group-hover:text-green-700 line-clamp-2 mb-2">{doc.title}</h3>

                    <div className="mt-auto pt-4 w-full">
                      <span className="inline-block px-4 py-1.5 rounded-full bg-green-50 text-green-600 text-sm font-medium border border-green-100 group-hover:bg-white group-hover:shadow-sm transition-all">
                        Đọc ngay
                      </span>
                    </div>
                  </div>
          ...
          <div className="bg-white w-full h-full max-w-6xl max-h-[90vh] rounded-2xl flex flex-col shadow-2xl overflow-hidden relative animate-scaleIn">
            ...
            <div className="bg-gray-900 text-white px-6 py-4 flex justify-between items-center shrink-0 shadow-lg z-10">
              <h3 className="font-bold text-xl truncate pr-4 flex items-center">
                <FileText size={20} className="mr-3 text-green-400" />
                {selectedDoc.title}
              </h3>
